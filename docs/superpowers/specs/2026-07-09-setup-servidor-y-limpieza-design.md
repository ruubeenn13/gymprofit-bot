...
Además (ampliación acordada): 🧑‍🏫 Coach, 🍎 Nutricionista, 🤖 Bots, 🎥 Creador, 🥇 Campeón del
mes, 📲 Vinculado, 🎂 Cumpleaños y **roles separadores** visuales (`▬▬ … ▬▬`) entre grupos. Lista
canónica en `SetupServidorPlan` (ver plan).

### Categorías y canales (separador `・`)

Enfoque realista: es una **comunidad** (charlar, jugar, compartir progreso), no un gimnasio en
voz. Fitness = canales de **conversación**; la voz es de **hangout/gaming**. La lista canónica y
actualizada vive en `SetupServidorPlan` (ver el plan de implementación).

| Categoría | Canales | Permisos |
|---|---|---|
| 📢 INFORMACIÓN | `bienvenidas` · `reglas` · `anuncios` · `cómo-funciona` · `novedades-app` | `@everyone`: ver sí, escribir no; staff escribe |
| 💬 COMUNIDAD | `general` · `presentaciones` · `memes` · `gaming` · `música` · `off-topic` | escribir sí |
| 🏋️ FITNESS | `ejercicio-del-día` · `progresos` · `fotos` · `nutrición` · `rutinas` · `dudas` | escribir sí |
| 🎮 GAMIFICACIÓN | `logros` · `ranking` · `economía` · `trivia` · `duelos` · `retos` · `comandos-bot` | escribir sí; `comandos-bot` slowmode 5 s |
| 🛎️ AYUDA | `sugerencias` · `soporte` | escribir sí |
| 🔊 VOZ | `General` · `Gaming` · `Gaming 2` · `Música` · `Chill` · `Privada` (límite 4) · `Directo` · `AFK` | por defecto |
| 🔒 STAFF | `staff-chat` · `bot-logs` · `moderación` · `reportes` · `logs-tickets` · `Voz staff` | `@everyone`: sin acceso; Staff: ver |
